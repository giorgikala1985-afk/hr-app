...
export async function generatePromotionPDF({ order, companyName, orderNumber }) {
  ensureGeorgianFont();
  ...
  const salaryChange = Number(order.newSalary) - Number(order.oldSalary);
  const salaryUp = salaryChange >= 0;

  const rows = [
    ['Order Reference',   orderRef],
    ['Order Type',        'Promotion Order'],
    ['Previous Position', order.oldPosition || '—'],
    ['New Position',      order.newPosition || '—'],
    ['Previous Salary',   order.oldSalary ? fmt(order.oldSalary) : '—'],
    ['New Salary',        order.newSalary ? fmt(order.newSalary) : '—'],
    ['Salary Change',     `${salaryUp ? '+' : ''}${fmt(salaryChange)}`],
    ['Effective Date',    order.effectiveDate || '—'],
    ['Notes',             order.notes || '—'],
  ];

  const html = `
<div id="order-pdf-root" style="width:560px;background:#ffffff;font-family:${FONT};color:#1e293b;box-sizing:border-box;">

  <!-- HEADER -->
  <div style="background:#0f3460;padding:16px 24px 14px;display:flex;justify-content:space-between;align-items:flex-start;">
    <div>
      <div style="color:#fff;font-size:14px;font-weight:700;margin-bottom:2px;font-family:${FONT};">${companyName}</div>
      <div style="color:#a8c4e0;font-size:9px;font-family:${FONT};">Human Resources Department</div>
    </div>
    <div style="text-align:right;">
      <div style="color:#fff;font-size:10px;font-weight:700;font-family:${FONT};">${orderRef}</div>
  ...
  <div style="margin:14px 24px 0;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px 14px;display:flex;justify-content:space-between;align-items:flex-start;">
    <div>
      ...
      <div style="font-size:13px;font-weight:700;color:#0f172a;margin-bottom:2px;font-family:${FONT};">${order.empName || '—'}</div>
    </div>
    <div style="text-align:right;">
      <div style="font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#94a3b8;margin-bottom:4px;font-family:${FONT};">DATE</div>
      <div style="font-size:11px;font-weight:700;color:#0f172a;font-family:${FONT};">${dateStr}</div>
    </div>
  </div>
  ...
  <div style="margin:12px 24px 0;">
    ...
    <div style="border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;">
      ${rows.map(([label, value], i) => `
        <div style="display:flex;justify-content:space-between;padding:6px 12px;background:${i % 2 === 0 ? '#f8fafc' : '#ffffff'};border-bottom:${i < rows.length - 1 ? '1px solid #f1f5f9' : 'none'};">
  ...
  try {
    ...
    pdf.save(`Promotion_${orderRef}_${(order.empName || 'Employee').replace(/\s+/g, '_')}.pdf`);
  } finally {

  ...
  const addFiles = (rawFiles) => {
    ...
    setTimeout(() => {
      const newEntries = rawFiles.map((f, i) => {
        ...
        return { id: Date.now() + i, name: f.name, category: 'HR', size, uploaded: new Date().toISOString().slice(0, 10), type: ext };
      });
      ...
      {activeSubTab === 'files' && (
        <>
          ...
          >
            ...
            {uploading ? (
              <div className="dl-uploading"><div className="dl-spinner" /><span>Uploading...</span></div>
            ) : (
              <>
                ...
                <p className="dl-drop-text"><strong>Drop files here</strong> or click to upload</p>
          ...
          <div className="dl-filters">
            <div className="dl-category-tabs">
              {CATEGORIES.map(cat => (
                <button key={cat} className={`dl-cat-btn${activeCategory === cat ? ' active' : ''}`} onClick={() => setActiveCategory(cat)}>{cat}</button>
              ))}
            </div>
            <div className="dl-search-wrap">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              <input className="dl-search" type="text" placeholder="Search files..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
          ...
          <div className="dl-file-list">
            {filtered.length === 0 ? (
              <div className="dl-empty">
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>
                <p>No files found</p>
              </div>
            ) : (
              filtered.map(f => (

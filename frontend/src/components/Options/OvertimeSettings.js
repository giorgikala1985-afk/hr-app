  ...
  return (
    <div style={{ background: 'var(--surface)', borderRadius: 14, border: '1px solid var(--border-2)', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
      ...
      <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-3)', background: 'var(--surface-2)', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text)' }}>Overtime Rates</div>
          <div style={{ fontSize: 12, color: 'var(--text-4)', marginTop: 1 }}>Configure percentage rates used in salary calculations</div>
        </div>
      ...
      <form onSubmit={handleAdd} style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-3)', display: 'flex', gap: 10, flexWrap: 'wrap', background: 'var(--surface)', alignItems: 'center' }}>
        ...
        <input
          ...
          min="1"
          step="0.1"
          style={{ width: 150, padding: '9px 12px', border: '1px solid var(--border-2)', borderRadius: 8, fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
          onFocus={e => e.target.style.borderColor = '#ca8a04'}
          onBlur={e => e.target.style.borderColor = 'var(--border-2)'}
        />
        <button
          ...
          style={{ padding: '9px 18px', background: saving || !label.trim() || !rate ? '#e2e8f0' : '#479c73', color: saving || !label.trim() || !rate ? '#94a3b8' : '#fff', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: saving || !label.trim() || !rate ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap', transition: 'all 0.15s' }}
        >
        ...
        <div>
          {rates.map((r, i) => (
            <div key={r.id} style={{ padding: '12px 24px', borderBottom: i < rates.length - 1 ? '1px solid #f8fafc' : 'none', display: 'flex', alignItems: 'center', gap: 10, transition: 'background 0.1s' }}
              onMouseEnter={e => { if (editId !== r.id) e.currentTarget.style.background = 'var(--surface-2)'; }}
            ...
            >
              {editId === r.id ? (
                ...
                <>
                  ...
                  <span style={{ flex: 1, fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{r.label}</span>
                  ...
                  <button onClick={() => startEdit(r)} title="Edit" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--border)', padding: 4, borderRadius: 6, transition: 'color 0.12s' }}
                    onMouseEnter={e => e.currentTarget.style.color = '#ca8a04'} onMouseLeave={e => e.currentTarget.style.color = 'var(--border)'}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                  </button>
                  <button onClick={() => handleDelete(r.id)} title="Delete" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--border)', padding: 4, borderRadius: 6, transition: 'color 0.12s' }}

  ...
  return (
    <div style={{ background: 'var(--surface)', borderRadius: 14, border: '1px solid var(--border-2)', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
      ...
      <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-3)', background: 'var(--surface-2)', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text)' }}>{t('pos.title')}</div>
          <div style={{ fontSize: 12, color: 'var(--text-4)', marginTop: 1 }}>{t('pos.desc')}</div>
        </div>
      ...
      <form onSubmit={handleAdd} style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-3)', display: 'flex', gap: 10, background: 'var(--surface)' }}>
        <input
          ...
          style={{ flex: 1, padding: '9px 12px', border: '1px solid var(--border-2)', borderRadius: 8, fontSize: 13, outline: 'none', boxSizing: 'border-box', transition: 'border-color 0.15s', background: 'var(--surface-2)', color: 'var(--text)' }}
      ...
      {loading ? (
        ...
        <div style={{ padding: '40px 24px', textAlign: 'center', color: 'var(--text-4)' }}>
          <div style={{ marginBottom: 8 }}><HugeiconsIcon icon={UserGroupIcon} size={32} color="#cbd5e1" strokeWidth={1.8} /></div>
        ...
        <div>
          {positions.map((pos, i) => (
            <div key={pos.id} style={{ padding: '12px 24px', borderBottom: i < positions.length - 1 ? '1px solid var(--border-3)' : 'none', display: 'flex', alignItems: 'center', gap: 10, transition: 'background 0.1s', background: 'var(--surface)' }}
              onMouseEnter={e => { if (editId !== pos.id) e.currentTarget.style.background = 'var(--surface-2)'; }}
              ...
              {editId === pos.id ? (
                <>
                  <input
                    ...
                    autoFocus
                    style={{ flex: 1, padding: '7px 10px', border: '1px solid #3b82f6', borderRadius: 7, fontSize: 13, outline: 'none', background: 'var(--surface-2)', color: 'var(--text)' }}
                  />
                  ...
                  <button onClick={cancelEdit} style={{ padding: '7px 12px', background: 'none', border: '1px solid var(--border-2)', borderRadius: 7, fontSize: 12, color: 'var(--text-3)', cursor: 'pointer' }}>{t('pos.cancel')}</button>
                </>
              ) : (
                <>
                  ...
                  <span style={{ flex: 1, fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{pos.name}</span>
                  <button onClick={() => startEdit(pos)} title={t('pos.edit')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--border)', padding: 4, borderRadius: 6, transition: 'color 0.12s' }}
                  ...
                  <button onClick={() => handleDelete(pos.id)} title={t('pos.delete')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--border)', padding: 4, borderRadius: 6, transition: 'color 0.12s' }}

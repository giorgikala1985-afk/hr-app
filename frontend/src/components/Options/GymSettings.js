  ...
  return (
    <div className="tab-panel">
      ...
      <div className="sc-form-card">
        <h4>{editingPlan ? t('gym.editPlan') : t('gym.addPlan')}</h4>
      ...
      {plans.length > 0 && (
        <div className="sc-form-card" style={{ marginTop: '16px' }}>
          ...
          <div className="gym-settings-plans">
            {plans.map((plan) => (
              <div key={plan.id} className="gym-settings-plan-item">
                <div className="gym-settings-plan-info">
                  <span className="gym-settings-plan-name">{plan.name}</span>
                  <span className="gym-settings-plan-meta">{plan.provider} &middot; {formatCurrency(plan.price)}/mo</span>
                </div>
      ...
      {plans.length > 0 && (
        <div className="sc-form-card" style={{ marginTop: '16px' }}>
          <h4>{t('gym.assignEmployeeToPlan')}</h4>
      ...
      <div style={{ marginTop: '24px' }}>
        <h4 style={{ color: '#1a1a2e', fontSize: '16px', marginBottom: '12px' }}>
          {t('gym.assignments').replace('{count}', assignments.length)}
        </h4>
        {assignments.length === 0 ? (
          <div className="sc-empty">{t('gym.noAssignments')}</div>
        ) : (
          <>
              ...
              <table className="emp-table">
                <thead>
                  <tr>
                    ...
                    <th>{t('gym.plan').replace(' *', '')}</th>
                    <th>{t('gym.provider').replace(' *', '')}</th>
            ...
            <div className="members-total">
              ...
              <span className="members-total-amount">{formatCurrency(totalMonthly)}</span>
            </div>
          </>
        )}
      </div>

...
function AiAgentTool() {
  ...
  return (
    <div style={{ background: 'var(--surface)', borderRadius: 14, border: '1px solid var(--border-2)', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.2)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '20px 24px', background: 'var(--surface-2)', borderBottom: '1px solid var(--border-2)', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text)' }}>AI Document Agent</div>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        ...
        {/* PDF Library sidebar */}
        <div style={{ width: 220, borderRight: '1px solid var(--border-2)', background: 'var(--surface-2)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          <div style={{ padding: '14px 16px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>PDF Library</span>
            <span style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 600 }}>{pdfLibrary.length}</span>
          </div>

          {/* File list */}
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px' }}>
            {pdfLibrary.map(f => (
              <div key={f.name} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 8px', borderRadius: 8, marginBottom: 2, background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--text-2)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={f.name}>{f.name}</span>
                <button
                  onClick={() => removeFile(f.name)}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: 0, fontSize: 15, lineHeight: 1, flexShrink: 0 }}
                  title="Remove"
                >×</button>
              </div>
            ))}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          <div style={{ flex: 1, minHeight: 320, maxHeight: 480, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            {!hasFiles && (
              <div style={{ textAlign: 'center', color: 'var(--text-3)', fontSize: 13, paddingTop: 60 }}>
            ...
            {messages.map((msg, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-3)', marginBottom: 4, paddingLeft: msg.role !== 'user' ? 2 : 0 }}>
                ...
                <div style={{
                  ...
                  background: msg.role === 'user' ? 'var(--accent, #3b82f6)' : msg.role === 'error' ? 'rgba(220,38,38,0.15)' : 'var(--surface-2)',
                  color: msg.role === 'user' ? '#fff' : msg.role === 'error' ? '#f87171' : 'var(--text)',
                  border: msg.role === 'assistant' ? '1px solid var(--border-2)' : 'none',
            ...
            {loading && (
              <div style={{ display: 'flex', alignItems: 'flex-start' }}>
                <div style={{ padding: '10px 14px', borderRadius: '14px 14px 14px 4px', background: 'var(--surface-2)', border: '1px solid var(--border-2)', display: 'flex', gap: 4, alignItems: 'center' }}>
                  {[0, 1, 2].map(d => (
                    <div key={d} style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--text-3)', animation: `bounce 1.2s ${d * 0.2}s infinite` }} />
                  ))}
                </div>
              </div>
            )}
          ...
          {/* Input */}
          <form onSubmit={handleAsk} style={{ borderTop: '1px solid var(--border-2)', padding: '16px 24px', display: 'flex', gap: 10 }}>
            <input
              value={question}
              ...
              style={{
                flex: 1, padding: '10px 14px', border: '1px solid var(--border-2)', borderRadius: 9,
                fontSize: 13, color: 'var(--text)', outline: 'none', background: (loading || !hasFiles) ? 'var(--surface-2)' : 'var(--surface)',
              }}
            ...
            <button
              ...
              style={{
                ...
                background: !question.trim() || loading || !hasFiles ? 'var(--surface-2)' : 'var(--accent, #3b82f6)',
                color: !question.trim() || loading || !hasFiles ? 'var(--text-3)' : '#fff',

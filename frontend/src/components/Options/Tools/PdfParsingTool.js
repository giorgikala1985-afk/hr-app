  ...
  return (
    <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
      ...
      <div style={{ padding: '20px 24px', background: '#fafbfc', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 15, color: '#1e293b' }}>PDF Parsing</div>
          <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 1 }}>Upload a PDF and convert it to Word or Excel</div>
        </div>
      </div>
      ...
      <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        {/* Messages */}
        {error && (
          <div style={{ padding: '10px 14px', background: '#fef2f2', color: '#dc2626', border: '1px solid #fca5a5', borderRadius: 8, fontSize: 13 }}>{error}</div>
        )}
        {success && (
          <div style={{ padding: '10px 14px', background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0', borderRadius: 8, fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 14, color: '#1e293b', marginBottom: 4 }}>Upload PDF File</div>
            <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 12 }}>Select or drag & drop your PDF. Max size 30 MB.</div>
            ...
            <div
              ...
              onDrop={handleDrop}
              style={{
                border: `2px dashed ${dragOver ? '#dc2626' : file ? '#fca5a5' : '#e2e8f0'}`,
                borderRadius: 12, padding: '28px 24px', textAlign: 'center', cursor: 'pointer',
                background: dragOver ? '#fef2f2' : file ? '#fef2f2' : '#fafbfc',
                transition: 'all 0.15s',
              }}
              onMouseEnter={e => { if (!dragOver) e.currentTarget.style.borderColor = '#94a3b8'; }}
              onMouseLeave={e => { if (!dragOver) e.currentTarget.style.borderColor = file ? '#fca5a5' : '#e2e8f0'; }}
            >
              {file ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                  ...
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#dc2626' }}>{file.name}</div>
                    <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 1 }}>{(file.size / 1024 / 1024).toFixed(2)} MB · click to replace</div>
                  </div>
                </div>
              ) : (
                <>
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: 10 }}>
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                    <polyline points="17,8 12,3 7,8"/><line x1="12" y1="3" x2="12" y2="15"/>
                  </svg>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#64748b' }}>Click to choose PDF</div>
                  <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}>or drag & drop here</div>
                </>
              )}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 14, color: '#1e293b', marginBottom: 4 }}>Choose Output Format</div>
            <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 12 }}>Select the format you want to convert to.</div>
              ...
              {FORMAT_OPTIONS.map(opt => (
                <button
                  ...
                  style={{
                    ...
                    border: `2px solid ${format === opt.key ? opt.color : '#e2e8f0'}`,
                    background: format === opt.key ? opt.bg : '#fafbfc',
                    transition: 'all 0.15s', minWidth: 180,
                  }}
                  onMouseEnter={e => { if (format !== opt.key) e.currentTarget.style.borderColor = '#94a3b8'; }}
                  onMouseLeave={e => { if (format !== opt.key) e.currentTarget.style.borderColor = '#e2e8f0'; }}
                >
                  {opt.icon}
                  <div style={{ textAlign: 'left' }}>
                    <div style={{ fontWeight: 700, fontSize: 13, color: format === opt.key ? opt.color : '#1e293b' }}>{opt.label}</div>
                    <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 1 }}>{opt.ext}</div>
                  </div>
                  {format === opt.key && (
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, fontSize: 14, color: '#1e293b', marginBottom: 4 }}>Convert & Download</div>
            ...
            <button
              ...
              style={{
                ...
                padding: '11px 28px',
                background: !file || converting ? '#e2e8f0' : selectedFormat.color,
                color: !file || converting ? '#94a3b8' : '#fff',

  ...
  return (
    <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
      ...
      <div style={{ padding: '20px 24px', borderBottom: '1px solid #f1f5f9', background: '#fafbfc', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 15, color: '#1e293b' }}>Departments</div>
          <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 1 }}>Organize employees by team or division</div>
        </div>
        ...
        <div>
          {departments.map((dept, i) => (
            <div key={dept.id} style={{ padding: '12px 24px', borderBottom: i < departments.length - 1 ? '1px solid #f8fafc' : 'none', display: 'flex', alignItems: 'center', gap: 10, transition: 'background 0.1s' }}
              onMouseEnter={e => { if (editId !== dept.id) e.currentTarget.style.background = '#f8fafc'; }}
            ...
            >
              {editId === dept.id ? (
                ...
                <>
                  ...
                  <span style={{ flex: 1, fontSize: 14, fontWeight: 500, color: '#1e293b' }}>{dept.name}</span>
                  <button onClick={() => startEdit(dept)} title="Edit" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#cbd5e1', padding: 4, borderRadius: 6, transition: 'color 0.12s' }}
                    onMouseEnter={e => e.currentTarget.style.color = '#7c3aed'} onMouseLeave={e => e.currentTarget.style.color = '#cbd5e1'}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                  </button>
                  <button onClick={() => handleDelete(dept.id)} title="Delete" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#cbd5e1', padding: 4, borderRadius: 6, transition: 'color 0.12s' }}

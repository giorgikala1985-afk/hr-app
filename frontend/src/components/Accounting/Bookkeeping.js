...
function Bookkeeping() {
  ...
  const handleTSave = async () => {
    ...
    if (validD.length === 0) { setTError('Add at least one debit entry.'); return; }
    if (validC.length === 0) { setTError('Add at least one credit entry.'); return; }
  ...
  const handleRemoveDuplicates = async () => {
    ...
    if (!window.confirm(`Remove ${toDelete.length} duplicate account(s)? Oldest entries will be kept.`)) return;
    setAccLoading(true); setAccError('');
    try {
      await Promise.all(toDelete.map(a => api.delete(`/accounting/bookkeeping-accounts/${a.id}`)));
      loadAccounts();
    } catch (err) {
      setAccError(err.response?.data?.error || 'Failed to remove duplicates.');
      setAccLoading(false);
    }
  };
      ...
      {view === 'transactions' && (
        <>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
            <input placeholder="Search description or account…" value={filterText} onChange={e => setFilterText(e.target.value)} style={{ flex: 1, minWidth: 200, ...inpStyle }} />
            ...
            <div style={overlay} onClick={() => setShowTxForm(false)}>
              <div style={{ ...modal, maxWidth: 680 }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 12, marginBottom: 16 }}>
                  ...
                  <div><label style={lbl}>Description *</label><input value={formDesc} onChange={e => setFormDesc(e.target.value)} placeholder="e.g. Office rent payment" style={inpStyle} /></div>
                </div>
                <datalist id="bk-accs">
                ...
                <div style={{ marginBottom: 20, position: 'relative' }}>
                  ...
                  {agentOpen && (
                    <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.1)', zIndex: 10, maxHeight: 180, overflowY: 'auto' }}>
                      ...
                      {agents.filter(a => a.name.toLowerCase().includes(agentSearch.toLowerCase())).length === 0 && <div style={{ padding: '8px 12px', color: '#94a3b8', fontSize: 13 }}>No agents found</div>}
                    </div>
                  )}
        ...
        <>
          ...
          : accounts.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#94a3b8' }}>
              ...
              <div style={{ fontSize: 13, marginTop: 4, marginBottom: 20 }}>Add accounts to use them in transactions.</div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                ...
                <tfoot>
                  <tr style={{ background: '#f1f5f9', borderTop: '2px solid #e2e8f0' }}>
                    <td style={td} /><td style={{ ...td, fontWeight: 700, color: '#1e293b', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1 }}>Totals</td>
                    ...
                    <td style={{ ...td, textAlign: 'right', fontFamily: 'monospace', fontWeight: 700, color: Math.abs(tbTotalDebit - tbTotalCredit) < 0.001 ? '#16a34a' : '#dc2626' }}>
                      {Math.abs(tbTotalDebit - tbTotalCredit) < 0.001 ? '✓ Balanced' : `Off by ${fmt(Math.abs(tbTotalDebit - tbTotalCredit))}`}
                    </td>
                  </tr>
                </tfoot>
              </table>
            ...
            <div style={{ padding: '20px 28px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 10, flex: 1, flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <input value={tDesc} onChange={e => setTDesc(e.target.value)} placeholder="e.g. Monthly rent payment" style={{ ...inpStyle, fontSize: 13 }} />
                </div>
                  ...
                  <div style={{ marginTop: 12, textAlign: 'center' }}>
                    {Math.abs(tDebitTotal - tCreditTotal) < 0.001 ? (
                      ...
                      <span style={{ fontSize: 12, fontWeight: 700, color: '#ea580c', background: '#fff7ed', border: '1px solid #fed7aa', padding: '4px 14px', borderRadius: 20 }}>
                        Difference: {fmt(Math.abs(tDebitTotal - tCreditTotal))} ({tDebitTotal > tCreditTotal ? 'Dr' : 'Cr'} side is higher)
                      </span>
                    )}
                  </div>
                )}
